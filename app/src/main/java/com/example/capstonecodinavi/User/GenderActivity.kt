...
class GenderActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // 저장된 성별 정보 가져오기
        val gender = sharedPreferences.getInt(getGenderKey(loginMethod), -1)
    ...
    private fun action() {
        ...
        binding.genderChangeBtn.setOnClickListener {
            ...
            if (index == 0) {
                Toast.makeText(this, "성별이 남성으로 설정되었습니다.", Toast.LENGTH_SHORT).show()
            }
            else if (index == 1) {
                Toast.makeText(this, "성별이 여성으로 설정되었습니다.", Toast.LENGTH_SHORT).show()
            }
            finish()
        }
